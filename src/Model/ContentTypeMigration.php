...
abstract class ContentTypeMigration /*extends ControllerBase*/{
   ...
   * @todo move in config form
   * Used for the data sources JSON file name and the migration class factory.
   * @var array
   */
  public static $sourceContentTypes =  array (
    'article' => 'Article',
    'page' => 'Page',
  );
  ...
  */
  ...
  {
    // @todo test batch
    //BatchProcessController::setBatch($this->entriesToMigrate);
    $count = 0;
    switch($this->sourceTranslationMode) {
      case ContentTypeMigration::TRANSLATION_MODE_UND:
        break;
      case ContentTypeMigration::TRANSLATION_MODE_I18N:
        foreach ($this->entriesToMigrate as $entry) {
          //kint($entry);
          ...
          if(ContentTypeMigration::DEBUG && $count == ContentTypeMigration::DEBUG_MAX_ITEMS) {
            break;
          }
          ++$count;
        }
